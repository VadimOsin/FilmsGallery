...
const Comment: React.FC<ICommentId> = ({idComment,title, text, user_id, deleteComment}) => {
    ...
    return (
        <div className="quote">
            ...
            {userInfo && userInfo.user_meta_id === parseInt(user.id) ?
                <button type="reset" className="addComments__btn__reset deleteComment" onClick={(event)=>deleteComment(idComment)}>DELETE</button> : null}
